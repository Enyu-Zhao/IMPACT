...
ROBOT_RADIUS = 2  # Simple circular robot footprint

PLANNER_PARAMS = {
    # simplified costs for plain A*
    "PUSH_BASE_COST": 8.0,
    "MOVE_COST": 1.0,
    ...
    "MAX_REGION_COST_FOR_ROBOT": 6.0,  # renamed from gripper
...
def get_robot_footprint_cached(center_r, center_c, radius=ROBOT_RADIUS):
    return get_robot_footprint_uncached(int(center_r), int(center_c), radius)


def get_robot_footprint_uncached(center_r, center_c, radius=ROBOT_RADIUS):
    """Get circular robot footprint"""
    footprint = set()
...
def swept_check_object_and_robot(state_map, obj_info, start_center, end_center, robot_start, robot_end, target_coords, safety_violation_map, distance_map, params=PLANNER_PARAMS):
    mask = obj_info['mask']
    ...
    for i in range(1, steps + 1):
        ...
        mid_robot = (robot_start[0] + (robot_end[0]-robot_start[0])*t,
                    robot_start[1] + (robot_end[1]-robot_start[1])*t)
        robot_fp = get_robot_footprint_cached(int(round(mid_robot[0])), int(round(mid_robot[1])))
        # Check safety buffer for robot during sweep
        buffer_violation, _ = check_safety_buffer_violation(robot_fp, safety_violation_map, distance_map, params)
        if buffer_violation:
            return False, 0.0, float('inf')
        ...
        for r, c in zip(rs, cs):
            ...
            if MIN_UNSAFE_OBJ <= v <= MAX_UNSAFE_OBJ:
                return False, 0.0, float('inf')
        for (robot_r, robot_c) in robot_fp:
            if not (0 <= robot_r < state_map.shape[0] and 0 <= robot_c < state_map.shape[1]):
                return False, 0.0, float('inf')
            v = state_map[robot_r, robot_c]
            if MIN_UNSAFE_OBJ <= v <= MAX_UNSAFE_OBJ:
                return False, 0.0, float('inf')
...
class Node:
    def __init__(self, r, c, obj_state, g=1e9, h=0, parent=None, action=None):
...
def get_actions(node, grid_shape, region_map, target_coords, safety_violation_map, distance_map):
    ...
    for dr in [-1, 0, 1]:
        for dc in [-1, 0, 1]:
            ...
            if not (0 <= nr < grid_shape[0] and 0 <= nc < grid_shape[1]):
                continue
            # For movement: validate robot placement
            valid, penalty = validate_robot_placement(nr, nc, current_map, region_map, safety_violation_map, distance_map)
            if not valid:
                continue
            base_cost = math.hypot(dr, dc) * PLANNER_PARAMS["MOVE_COST"]
            total_cost = base_cost + penalty
            actions.append(('move', (nr, nc), total_cost, None))

    # No rotation actions for plain A*

    # Push generation: simplified without angle considerations
    ...
    for rr in range(rmin, rmax + 1):
        for cc in range(cmin, cmax + 1):
            ...
            dist_to_robot = math.hypot(rr - node.r, cc - node.c)
            push_value += max(0, 10 - dist_to_robot)
    ...
    for push_value, rr, cc, safety_score, obj_id, obj_info, centroid in push_candidates[:max_pushes]:
        ...
        # Generate simplified push directions (no angle considerations)
        ...
        if normal_vec is None:
            ...
            # Simplified: just use normal direction and a few variations
            dir_candidates = [normal_ang, (normal_ang + 45) % 360, (normal_ang - 45) % 360]
        ...
        for ang in selected_angles:
            ...
            actions.append(('push', (node.r, node.c), total_cost, push_meta))

    return actions


def validate_robot_placement(robot_r, robot_c, base_map, region_map, safety_violation_map, distance_map, params=PLANNER_PARAMS):
    fp = get_robot_footprint_cached(robot_r, robot_c)
    total_region_cost = 0.0
    total_object_penalty = 0.0

    # NEW STRICT RULE FOR HIGH-COST OBJECTS
    high_cost_mask = region_map > params["HIGH_COST_THRESHOLD"]
    if params.get("STRICT_HIGH_COST_BUFFER", True):
        # Expand high-cost areas by fixed distance (8 pixels here)
        strict_buffer = binary_dilation(high_cost_mask,
                                        iterations=params.get("HIGH_COST_STRICT_DISTANCE", 8))
        for (fr, fc) in fp:
    ...
    for (fr, fc) in fp:
        ...
        if region_cost > params["MAX_REGION_COST_FOR_ROBOT"]:
            return False, float('inf')
...
def plain_a_star_planner(start_coords, goal_coords, base_map, region_map):
    ...
    valid_start, _ = validate_robot_placement(start_coords[0], start_coords[1], 
    ...
    start_node = Node(start_coords[0], start_coords[1], initial_state, g=0,
    ...
    while open_heap:
        ...
        for act_type, next_conf, base_cost, meta in actions:
                    ...
                    if obj_info is None:
                        ...
                        if start_center is None:
                            ...
                            if displacement_dist < PLANNER_PARAMS["MIN_PUSH_DISTANCE"]:
                                ...
                                robot_start = (cur.r, cur.c)
                                robot_end = (robot_start[0] + mean_disp[0], robot_start[1] + mean_disp[1])
                                ok, disturbance, _ = swept_check_object_and_robot(new_state.get_current_map(), obj_info, start_center, end_center,
                                                                                 robot_start, robot_end, goal_coords, safety_violation_map, distance_map)
                                if not ok:
                                    success = False
                                else:
            ...
            valid_fp, fp_penalty = validate_robot_placement(nr, nc, new_map, region_map, safety_violation_map, distance_map)
            if not valid_fp or fp_penalty == float('inf'):
                continue
            tentative_g = cur.g + base_cost + added_cost + fp_penalty
            h = heuristic(nr, nc, goal_r, goal_c)
            new_node = Node(nr, nc, new_state, g=tentative_g, h=h, parent=cur, action=action_description)
...
def visualise_path(base_map, region_map, path, start_coords, goal_coords, total_cost, data_dir="", filename=""):
    ...
    if path:
        ...
        for i in step_indices:
            r, c = path[i]['position']
            fp = get_robot_footprint_cached(int(round(r)), int(round(c)))
            for fr, fc in fp:
                if 0 <= fr < base_map.shape[0] and 0 <= fc < base_map.shape[1]:
...
def plan_path_for_scene(base_map, region_map, start_coords, start_angle=0, log_dir="", filename=""):
    ...
    path, total_cost = plain_a_star_planner(
        start_coords, goal_coords, base_map, region_map)
...
class PlainAStar:
    ...
    def find_path(self, start_coords: tuple = (97, 50)):
